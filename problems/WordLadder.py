...
class Solution(object):
    ...
    def ladderLength(self, beginWord, endWord, wordList):
        ...
        while len(q)>0:
            ...
            if curr_word in word_dict:
                word_dict.pop(curr_word)
            # ///////////This part i.e iterating over each word everytime and checking
            # takes too much time and causes TLE

            for word in word_dict.keys():
                if self.is_one_change_word(curr_word,word):
                    q.append(word)
                    adj_list.append(word)

                    if word==endWord:
                        is_found=True
            graph[curr_word]=adj_list

            # //////////////////////////////////
            if is_found:
                break

        self.getShortestPath(beginWord,endWord,graph,2)
        return self.ans



    def is_one_change_word(self,word1,word2):
        sz=len(word1)
        ctr=0
        for i in range(sz):
            if word1[i]==word2[i]:
    ...
    def getShortestPath(self,start,end,graph,level):
        if graph.get(start) and end in graph[start]:
            self.ans=min(self.ans,level)
            return level
        if graph.get(start):
            for elem in graph[start]:
                self.getShortestPath(elem,end,graph,level+1)
        return level


    

    def ladderLength_smart(self, beginWord, endWord, wordList):
        """
        :type beginWord: str
        :type endWord: str
        :type wordList: List[str]
