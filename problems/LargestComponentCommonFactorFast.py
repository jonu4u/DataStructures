...
# This solution is copied from LC. Here we take a hashmap and continue adding factors of each number to a map and while doing so also union of that factor with index.
#     This ensures that each distinct factor is placed in the map and since each index is unioned, we eventually unionfy those groups with common factors.
# So in a single loop iteration the process is complete
...
class UnionFind:
    ...
    def union(self,x,y):
        ...
        if size1>size2:
            ...
            self.parent[py]=px
            self.max_val=max(self.max_val,self.size[px])
        else:
            self.size[py] += self.size[px]
            self.parent[px]=py
            self.max_val=max(self.max_val,self.size[py])
